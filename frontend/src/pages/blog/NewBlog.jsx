...
const FormS = ({ data, close, setList }) => {
    ...
    const onSubmitForm = async (values) => {
        let response;
        console.log(values)
        let newVal = {...values,id:(values.title+values.email+values.title)}
        response = await axios.post('http://localhost:4000/api/blog/create', newVal);
                    ...
                    {({ handleSubmit, handleChange, handleBlur, values, touched, errors }) => (
                        <Form noValidate onSubmit={handleSubmit} className="w-75 d-flex flex-column justify-content-center">
                            ...
                            <Form.Group as='div' className="position-relative mb-5 cf-title-12">
                                <Form.Label className="w-100 text-center h1" >Tạo bài viết</Form.Label>
                            </Form.Group>
