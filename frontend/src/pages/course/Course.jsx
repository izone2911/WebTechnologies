...
function Course() {
    ...
    const ParseCSV_Exercise = (option) => {
        const fileInput = document.getElementById((option === 0) ? 'inputFile' : 'inputFilex');
        ...
        if (file && file.type === 'text/csv') {
          ...
          reader.onload = async (e) => {
            ...
            if (option === 0)
                ...
                await axios.post("http://localhost:4000/api/exercise/create",jsonData);
    ...
    const ParseCSV_Exam = (option) => {
        const fileInput = document.getElementById((option === 0) ? 'inputFileExam' : 'inputFileExamx');
        const file = fileInput?.files[0];
        ...
        if (file && file.type === 'text/csv') {
          ...
          reader.onload = async (e) => {
            ...
            if(option === 0)
                ...
                await axios.post("http://localhost:4000/api/exam/create",jsonData) 
                window.location.reload();
            }
            console.log("Success")
          };
    ...
    function deleteE(option, id) {
        if (window.confirm('Are you sure you want to save this thing into the database?')) {
            const func = async () => {
                if (option === 0) await axios.post("http://localhost:4000/api/exercise/delete/", {exerciseID: id});
                else await axios.post("http://localhost:4000/api/exam/delete/", {examID: id});
            }
            func();
            window.location.reload();
          } else {
        ...
        return(
            <div style={{marginLeft:"50px",marginRight:"50px"}} className="container2">
                ...
                <div className="kim-header">
                    ...
                    <button className="btnxnxx1" style={{marginTop:"15px"}} data-toggle="dropdown">+</button>
                    <ul className="kim-dropdown-menu dropdown-menu">
                        <li>
                            <input type="file" onChange={()=>ParseCSV_Exam(1)} id='inputFileExamx' accept=".csv" style={{display:"none"}}/>
                            <button style={{borderRadius:"15px",marginLeft:"5px"}} className="setting-button" onClick={()=>{document.getElementById('inputFileExamx').value="";document.getElementById('inputFileExamx').click()}}>Exam</button>
                        </li>
                        <li>
                            <input type="file" onChange={()=>ParseCSV_Exercise(1)} id='inputFilex' accept=".csv" style={{display:"none"}}/>                                    
                            <button style={{borderRadius:"15px",marginLeft:"5px"}} className="setting-button" onClick={()=>{document.getElementById('inputFilex').value="";document.getElementById('inputFilex').click()}}>Exercise</button>
                        </li>
                    </ul>
                </div>
                <p style={{fontFamily:"Verdana",fontSize:"20px"}}>{course.description}</p>

                <div className="hellobannho" style={{display:"flex", width: "100%", flexWrap: "wrap", justifyContent:"space-between"}}>
                    {exercises.map((quiz, index) => (
                        <li key={index} className="quiz-item kell">
                            <div className="all-div-ex">
                                <div className="div-image">
                                    ...
                                    <div className="quiz-info">
                                        ...
                                        {acc.role === 'Teacher'?
                                        <>
                                        <input type="file" onChange={()=>ParseCSV_Exercise(0)} id='inputFile' accept=".csv" style={{display:"none"}}/>
                                ...
                                <div className="div-delete">
                                    ...
                                    <motion.button
                                        ...
                                        onClick={()=>deleteE(0, quiz.exerciseID)}>
                                ...
                                <div className="all-div-ex">
                                    <div className="div-image">
                                        ...
                                        <div className="exam-info">
                                            ...
                                            {acc.role === 'Teacher'?
                                            <>
                                            <input type="file" onChange={()=>ParseCSV_Exam(0)} id='inputFileExam' accept=".csv" style={{display:"none"}}/>
